...
class DataParser:
    ...
    def add_data(self, file_path: str) -> int:
        ...
        for tx in existing_transactions:
            ...
            date_str = str(tx[0]).split(' ')[0]
            isin_val = tx[6]
            asset_name = tx[3]
            key = (date_str, tx[1], tx[2], isin_val if (isin_val and isin_val.strip()) else asset_name)
        ...
        for transaction in avanza_data:
            ...
            isin_val = row[9]
            asset_name = row[3]
            key = (date_str, row[1], row[2], isin_val if (isin_val and isin_val.strip()) else asset_name)
